...
public class DepthFirstIterativeTrieDictionarySolver implements BoardSolver {
    ...
    private void buildSolutionSet(
        ...
        if (xx >= 0
                && yy >= 0
                && xx < board.getRowLength()
                && yy < board.getRowLength()
                && !visited[xx][yy]) {
    ...
    private static boolean[][] copyArray(final boolean[][] visited, final Board board) {
